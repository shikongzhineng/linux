...
static struct mwv207_job *mwv207_ttm_job_alloc(struct ttm_buffer_object *bo)
{
	...
	dma_cmd->height = 1;
	dma_cmd->src.stride = bo->resource->num_pages << PAGE_SHIFT;
	dma_cmd->dst.stride = bo->resource->num_pages << PAGE_SHIFT;
	dma_cmd->width = bo->resource->num_pages << PAGE_SHIFT;
				...
				struct ttm_resource *new_mem)
{
	...
	dma_cmd->src.pg_nr_type = MWV207_DMA_NR_PAGES_VRAM(bo->resource->num_pages);
				...
				struct ttm_resource *new_mem)
{
	...
	dma_cmd->dst.pg_nr_type = MWV207_DMA_NR_PAGES_VRAM(bo->resource->num_pages);
